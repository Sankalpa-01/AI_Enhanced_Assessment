...
#     """
#     Generates a report file in PDF or JSON.
#     """
#     os.makedirs("reports", exist_ok=True)
    
#     # Use .pdf or .json extension properly
...
#         textobject.textLine(f"Student Name: {student_info.get('name', 'Unknown')}")
#         textobject.textLine(f"Roll Number: {student_info.get('roll_no', 'Unknown')}")
#         textobject.textLine("")
#         textobject.textLine(f"Meta Info: {report_meta or 'N/A'}")
#         textobject.textLine("")
#         textobject.textLine(f"Evaluation Summary: {result.get('evaluation_summary')}")
#         textobject.textLine(f"Score: {result.get('overall_score')} / {result.get('total_questions')}")
#         textobject.textLine(f"Strengths: {result.get('strengths')}")
#         textobject.textLine(f"Areas for Improvement: {result.get('areas_for_improvement')}")
#         textobject.textLine("")
#         textobject.textLine("Detailed Breakdown:")
#         textobject.textLine("")

#         for q in result.get("detailed_breakdown", []):
#             line = f"Q{q['question_number']}: Student Answer: {q['student_answer']} | Correct: {q['correct_answer']} | Correct? {q['is_correct']}"
